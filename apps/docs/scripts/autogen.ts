...
// 1. Get all the components in the components folder
const componentsFolder = resolve(__dirname, "../../../packages/wedges-vue/src/components");
const componentFolders = readdirSync(componentsFolder, { withFileTypes: true }).filter(dirent => dirent.isDirectory());

componentFolders.forEach((folder) => {
  const folderPath = join(componentsFolder, folder.name);
  const files = readdirSync(folderPath, { withFileTypes: true }).filter(file => file.name.includes(".vue"));
  components[folder.name] = files.map(file => parse(file.name).name);
});
...
const tsconfigChecker = createComponentMetaChecker(
  resolve(__dirname, "../../../packages/wedges-vue/tsconfig.json"),
  checkerOptions,
);
...
const allComponents = fg.sync(["src/**/*.vue", "!src/**/story/*.vue", "!src/**/*.story.vue"], {
  cwd: resolve(__dirname, "../../../packages/wedges-vue"),
  absolute: true,
});
...
primitiveComponents.forEach((componentPath) => {
  ...
  const metaDirPath = resolve(__dirname, "../src/content/meta");
...
function getEventFromComponentPath(dir: string) {
  const files = readdirSync(resolve(__dirname, "../../../packages/wedges-vue/src/components", dir), { withFileTypes: true }).filter(file => file.name.includes(".vue"));
